	...
	while (true) {
		...
		if (recv(s, buffer, sizeof(buffer), 0) > 0) {
			...
			if (strcmp(buffer_temp_1, "pp") != 0) {
				...
				if (roomptr.checkKickUsername(username))
...
void Session::processMessage(char message[1024], SOCKET s) {
	...
	if (splitedMessage.size() != 0) {
		...
		std::string username = connections[s];
		if (roomptr.getType(username).compare("admin") == 0) {
			if (first->compare("/ban") == 0)
				roomptr.addKickedUsername(second->data());
			else if (first->compare("/unban") == 0)
				roomptr.removeKickedUsername(second->data());
			else if (first->compare("/info") == 0) {
				char data[1024];
				strcpy_s(data, db.getUserInfor(username));
				strcat_s(data, ",Type: ");
				strcat_s(data, roomptr.getType(username).c_str());
				puts(data);
				send(s, data, sizeof(data), 0);
			}
			else if (first->compare("/mod") == 0)
				roomptr.giveMod(second->data());
			else if (first->compare("/unmod") == 0)
				roomptr.removeMod(username);
		}
		else if (roomptr.getType(username).compare("admin") == 0 || roomptr.getType(username).compare("mod") == 0) {
			...
			for (it = connections.begin(); it != connections.end(); ++it) {
				...
				if (username.compare(it->second) != 0 && roomptr.checkKickUsername(it->second)) {
...
void Session::removeSocket(std::string username) {
	...
	std::cout << "Having " << connections.size() << " client(s) connecting" << std::endl;
}